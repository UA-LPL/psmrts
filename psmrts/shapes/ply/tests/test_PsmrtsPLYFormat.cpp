...
TEST_CASE ( "PLY FORMAT Asset Test - Default Constructor", "[format][ply][default]") {
    ...
    */
    ...
    CHECK( meta_data.find_metadata("ply_file_type").to_string()  == "binary" );
    CHECK( meta_data.find_metadata("n_vertices").to_string()     == "1348" );
    CHECK( meta_data.find_metadata("n_facets").to_string()       == "2692" );
    CHECK( meta_data.find_metadata("minimum_radius").to_string() == "0.224243018");
    CHECK( meta_data.find_metadata("maximum_radius").to_string() == "0.289774341" );
}
