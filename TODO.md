...
- [x] Devise strategy to support independent testing and code coverage.
  - [x] Get code coverage actually reporting results.
...
- [x] Add `PSMRTS` facet request C API functions.
...
- [x] Add `explicit` to constructors with just a string and another with just an ordered json object. They conflict. There could be others. Best practice is use explicit when in doubt.
...
- [x] Major cleanup of CMake, code set. For example, completely eliminate BulltTracerModel.hpp - its not needed.
  - [x] Identify and remove obsolete/unused code.
  - [x] Remove/consolidate tests to proper code directories. For example, integrate the naifdsk/tests/test_PsmrtsPriorityTracer.cpp into the core tests. Same for test_PsmrtsRayTrace.cpp.
  - [x] Remove all instances of virtual inheritance were not needed. 
  - [ ] Consider renaming files to remove/limit `Psmrts` redundancy.
- [x] Rework PSMRTS include file strategy to use full paths to make external installs, maintanence and usage.
...
- [x] Remove line 15 in the test_PsmrtsShapeTracer.cpp as its not used in the test. 
...
- [ ] Add volume and surface area calculations for facets.
