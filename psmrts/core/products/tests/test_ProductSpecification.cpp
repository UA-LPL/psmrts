...
TEST_CASE ( "ProductSpecification Constructor / Base Function Test", "[product][specification][constructor][base]" ) {
    ...
    // This runs the spec extraction method
    auto cart_c = psmrts::ProductCart::extract_config( config1, product1 );
    CHECK( cart_c.isvalid()       == true );
    CHECK( cart_c.size()          == 3 );
    CHECK( cart_c.residual_size() == 1 );
    CHECK( cart_c.error_count()   == 0 );
}
