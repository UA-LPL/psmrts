...
TEST_CASE("PsmrtsBufferData Value Array Test", "[buffer][value][array]") {
    ...
    for (int i = 0; i < pb_data.size(); i++ ) {
        ...
        CHECK ( pb_data.data_get(i) == &my_vector[i] ); //get usually associated w/ ptr
    }
    ...
    psmrts::PsmrtsBufferData pb_cb_slice ( pb_data.slice(2, 4) ); // Second param is # of values you want to slice, assumed to be contiguous
    ...
    CHECK( pb_copy.track_count() == 0 ); // Should be 1?
    ...
    CHECK( pb_copy.track_count() == 0 ); // Currently not counting, should it be invoked and how?
