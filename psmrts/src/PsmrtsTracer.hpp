...
namespace psmrts {
  ...
  class PsmrtsTracer : public ProductProcessDispatch< MissingProcessRequestHandler, EllipsoidShapeTracer, BulletShapeTracer, NaifDskShapeTracer> {
    public:
      ...
      inline static PsmrtsTracer sphere( const double radius_km, const std::string &name="sphere" ) {
        ...
        return ( PsmrtsTracer( EllipsoidShapeTracer( radii, name ) ) );
      }
      ...
      inline static PsmrtsTracer bullet( const std::string &meshfile ) {
        ...
        if ( "obj" == fext_t ) {
          return ( PsmrtsTracer( BulletShapeTracer( psmrts::bullet::PsmrtsBulletWorldModel( psmrts::bullet::PsmrtsBulletMeshMap ( psmrts::PsmrtsOBJFormat( meshfile ) ), meshfile ) ) ) );
        }
        else if ( "ply" == fext_t ) {
          ...
          return ( PsmrtsTracer( BulletShapeTracer( bt_world ) ) );
        }
        else {  // ( "dsk" == fext_t )
      ...
      inline static PsmrtsTracer naifdsk( const std::string &dskfile ) {
        return ( PsmrtsTracer( NaifDskShapeTracer( dskfile ) ) ); 
      }
