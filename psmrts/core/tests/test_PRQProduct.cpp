...
TEST_CASE( "PRQProduct Default", "[prq][product][default]") {
  ...
  // Create an OBJ tracer via configuration and ensure it manifests in the PRQ.
  ...
  // Add the inventory to the PRQ. NOTE this constructor adds this inventory
  // to the local innventory and not the product inventory. The local
  // inventory will be searched first to resolve config request before
  // the system (factory) inventory is searched to build the product
  // inventory. This restricts searches to a pool of resources specifed
  // by the user and does not allow/provide for further updates to the local
  // inventory.
  ...
  // This check ensures the tracer was added to the product incentory
